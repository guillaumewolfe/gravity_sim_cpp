...
void main() {
    ...
    vec3 baseBlue = vec3(0.0, 0.0, 1.0); // Bleu de base
    vec3 baseRed = vec3(1.0, 0.1, 0.0); // Rouge orangé de base

    // Mélange des couleurs de base avec du blanc pour les adoucir
    vec3 rayleighColor = mix(baseBlue, vec3(1.0, 1.0, 1.0), 0.4); // Bleu moins intense
    vec3 sunsetColor = mix(baseRed, vec3(1.0, 1.0, 1.0), 0.1); // Rouge moins intense

    // Adoucissement de la transition
    float edgeWidth = 0.4; // Largeur de la zone de transition
    ...
    gl_FragColor = (vec4(finalColor, alpha/1.5) * diffuseColor * 1.5) + (vec4(finalColor, alpha/1.5) * ambientColor);
}
