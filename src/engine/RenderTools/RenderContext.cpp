...
std::vector<Button*>& btns,std::vector<ImageButton*>& imgbtns, float* maxsize, bool* showaxe, SystemeSolaire* sysSol, int* speedIndice, std::vector<std::pair<int, std::string>> speedList,bool* iscreating, bool* showInfo, bool* showCameraOptions,bool* isLive, bool* showOptions, bool* showSettings, bool* isOrbiting, bool* isPaused )
    : simulationTime(time),timeMultiplier(tmul) ,currentCamera(currentCam),
    labels(lbls), buttons(btns),imageButtons(imgbtns), maxSize(maxsize), showAxes(showaxe), systemeSolaire(sysSol), speedSettings(speedList), currentSpeedIndex(speedIndice),isCreating(iscreating),showInfo(showInfo), showCameraOptions(showCameraOptions), isLive(isLive), showOptions(showOptions), showSettings(showSettings), isOrbiting(isOrbiting), isPaused(isPaused) {
