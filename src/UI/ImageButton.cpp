...
void ImageButton::loadTexture(const char* relativeFilename) {
    std::string fullPath = getFullPath(relativeFilename);
    cv::Mat image = cv::imread(fullPath, cv::IMREAD_UNCHANGED);
    if (image.empty()) {
        std::cerr << "Erreur: Image non trouvée " << relativeFilename << std::endl;
        return;
    }
